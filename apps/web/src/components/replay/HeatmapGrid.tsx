...
const LEGEND_STEPS = [0.08, 0.2, 0.35, 0.5, 0.65]

function depthToOpacity(depth: number, max: number): number {
  if (max === 0) return 0.04
  return 0.08 + 0.55 * (depth / max)
}

export function HeatmapGrid({ cells, peakHours }: HeatmapGridProps) {
  const maxDepth = Math.max(...cells.map((c) => c.depth), 1)
  const width = 24 * (CELL_SIZE + CELL_GAP) + GRID_LEFT_OFFSET
  const height = 7 * (CELL_SIZE + CELL_GAP) + HOUR_LABEL_ROW_HEIGHT

  return (
    <div data-testid="heatmap-grid" style={{ overflowX: 'auto' }} className="scroll-thin">
      <svg width={width} height={height} style={{ minWidth: width }}>
        {Array.from({ length: 8 }, (_, i) => i * 3).map((hour) => (
          <text key={hour} x={GRID_LEFT_OFFSET + hour * (CELL_SIZE + CELL_GAP)} y={height - 3} fontSize={8.5} fill="var(--text-tertiary)" fontFamily="var(--font-mono)">
            {hour}
          </text>
        ))}
        ...
        {DAY_LABELS.map((label, i) => (
          <text key={label} x={0} y={i * (CELL_SIZE + CELL_GAP) + CELL_SIZE - 1} fontSize={9} fill="var(--text-tertiary)" fontFamily="var(--font-mono)">
            {label}
          </text>
        ))}
      </svg>

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 10, minWidth: width }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          {LEGEND_STEPS.map((o, i) => (
            <span key={i} aria-hidden="true" style={{ width: 9, height: 9, borderRadius: 3, background: 'var(--accent)', opacity: o }} />
          ))}
