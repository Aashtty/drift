...
export function RecentSessionsList({ sessions, taskNameById, anchorForTask, limit = 8 }: RecentSessionsListProps) {
  const sorted = [...sessions].sort((a, b) => new Date(b.started_at).getTime() - new Date(a.started_at).getTime()).slice(0, limit)
        ...
        return (
          <div key={s.id} className="glass" style={{ padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, borderLeft: anchor ? `2px solid ${anchor.color}` : undefined }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
              {s.flow_detected && (
                <span className="text-micro-mono" style={{ padding: '2px 6px', borderRadius: 'var(--radius-sm)', background: 'var(--surface-active)', color: 'var(--accent-b)' }}>flow</span>
              )}
              {s.hyperfocus && (
                <span title="locked in" style={{ display: 'flex', color: 'var(--accent)', padding: '2px 5px', borderRadius: 'var(--radius-sm)', background: 'var(--surface-active)' }}>
                  <LockIcon />
                </span>
              )}
