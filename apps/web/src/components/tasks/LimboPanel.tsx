...
interface LimboPanelProps {
  ...
  /** Renamed from `onKillAll` — the button now reads "Archive All" to
   *  say what it actually does, and the prop follows suit. */
  onArchiveAll: () => void
}
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
        ...
        <button
          ...
          style={{ padding: '8px 14px', borderRadius: 9, border: '1px solid var(--border-accent)', background: 'transparent', color: 'var(--text-secondary)', fontSize: 12.5, fontWeight: 500, cursor: 'pointer', whiteSpace: 'nowrap' }}
        >
          Archive
        </button>
      </div>
...
export function LimboPanel({ open, onClose, tasks, onRestore, onArchive, onArchiveAll }: LimboPanelProps) {
  ...
  return (
    <>
            ...
            >
              <div
                ...
                style={{
                  ...
                  background: `linear-gradient(135deg, color-mix(in srgb, var(--accent) 26%, var(--bg)) 0%, color-mix(in srgb, var(--bg) 94%, black) 40%, color-mix(in srgb, var(--accent) 18%, var(--bg)) 75%, color-mix(in srgb, var(--bg) 95%, black) 100%)`,
                  backgroundSize: '240% 240%', animation: 'limbo-gradient-shift 12s ease infinite',
                }}
              />
              <div
                style={{
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <p style={{ fontSize: 11, letterSpacing: '0.08em', color: 'var(--text-secondary)', margin: 0 }}>LIMBO {tasks.length > 0 && `· ${tasks.length}`}</p>
                    <p style={{ marginTop: 4, fontSize: 12, color: 'var(--text-tertiary)', maxWidth: 380 }}>Tasks you parked, or that quietly went stale.</p>
                  </div>
                  <button
                    type="button"
                    onClick={onClose}
                ...
                <div className="limbo-scroll-area" style={{ display: 'flex', flexDirection: 'column', gap: 8, overflowY: 'auto', overflowX: 'hidden' }}>
                  {tasks.length === 0 && <p style={{ padding: '28px 0', textAlign: 'center', fontSize: 13, color: 'var(--text-tertiary)' }}>Limbo is empty.</p>}
                  {tasks.map((t) => (
                    <LimboRow key={t.id} task={t} onRestore={onRestore} onArchive={onArchive} onDragStateChange={handleDragStateChange} />
                  ))}
                </div>
      ...
      <style>{`
        ...
        .limbo-scroll-area::-webkit-scrollbar { width: 5px; }
        .limbo-scroll-area::-webkit-scrollbar-track { background: transparent; }
        .limbo-scroll-area::-webkit-scrollbar-thumb { background: var(--border-accent); border-radius: 999px; }
      `}</style>
    </>
  )
}
